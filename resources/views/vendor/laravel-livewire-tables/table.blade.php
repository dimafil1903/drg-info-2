    ...
    <div class="row justify-content-between">
        <div class="col-auto order-last order-md-first">
            <div class="input-group mb-3 ">
                <input type="search" class="form-control" placeholder="{{ __('Пошук') }}" wire:model="search">
            </div>

        </div>
        <div class="col-auto order-last order-md-first d-flex flex-row-reverse bd-highlight mb-3 ">

            <button class="btn btn-danger" @if(empty($this->checkbox_values)) disabled @endif onclick="confirm('Are you sure?') || event.stopImmediatePropagation();" wire:click="deleteChecked">
                Видалити обрані
            </button>
        </div>
        @if($header_view)
